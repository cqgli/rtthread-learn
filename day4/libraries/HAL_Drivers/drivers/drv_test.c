...
#include <rtdevice.h>
    ...
    #include <ulog.h>
    static rt_err_t dev_test_init(rt_device_t devg)
    {
        LOG_I("test dev init");
        return RT_EOK;
    }
    ...
    {
        LOG_I("test dev open oflag = %d" , oflag);
        return RT_EOK;
    }
    ...
    static rt_err_t dev_test_write(rt_device_t dev,rt_off_t pos,void *buffer,rt_size_t size)
    {
    ...
    int rt_hw_test_init(void)
    {
        rt_device_t test_dev = rt_device_create(RT_Device_Class_Char,0);
        if(!test_dev)
        {
            LOG_E("test dev create falied");
            return RT_EOK;
        }
    ...
    INIT_BOARD_EXPORT(rt_hw_test_init);
